process CHECK_PRIMERS {
  ...
  conda "${projectDir}/envs/trimmer.yml"
  ...
  for idx in "\${!arr_ids[@]}"; do
      ...
      matches_fwd=\$(grep -E "Reads with adapters" "\${pid}.fwd.log" | sed -E 's/.*: *([0-9,]+).*/\\1/' | tr -d ',')
      percent_fwd=\$(grep -E "Reads with adapters" "\${pid}.fwd.log" | sed -E 's/.*\\((.*)%\\).*/\\1/')
      total_fwd=\$(grep -E "Total reads" "\${pid}.fwd.log" | sed -E 's/.*: *([0-9,]+).*/\\1/' | tr -d ',')
      ...
      matches_rev=\$(grep -E "Reads with adapters" "\${pid}.rev.log" | sed -E 's/.*: *([0-9,]+).*/\\1/' | tr -d ',')
      percent_rev=\$(grep -E "Reads with adapters" "\${pid}.rev.log" | sed -E 's/.*\\((.*)%\\).*/\\1/')
      total_rev=\$(grep -E "Total reads" "\${pid}.rev.log" | sed -E 's/.*: *([0-9,]+).*/\\1/' | tr -d ',')
